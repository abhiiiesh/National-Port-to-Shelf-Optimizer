...
export function NewsPage(): JSX.Element {
  ...
  const effectiveRoute = resolveEffectiveRoute(
    bulletins.map((bulletin) => bulletin.severity),
    activeRouteId,
    escalationRoutes
  );
  const isAutoRouted = criticalCount > 0;
        ...
        <div className="stack-list">
          {filteredBulletins.map((bulletin) => (
            <div className="approval-card" key={bulletin.id}>
              ...
              <div className="kpi-label">
                Preview:{' '}
                {buildBulletinPreview(selectedTemplateId, communicationTemplates, bulletin.title)}
              </div>
              <div className="kpi-label">
