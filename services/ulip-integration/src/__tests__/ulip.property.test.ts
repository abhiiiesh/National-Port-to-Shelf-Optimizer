...
describe('ULIP Integration properties (Task 17)', () => {
  ...
  test('Property 69: Customs Status Query', async () => {
    const service = createService();
    const status = await service.getCustomsStatus('CONT-CUST-1');

    expect(status.containerId).toBe('CONT-CUST-1');
    expect(status.status).toBeDefined();
  });

  test('Property 70: Customs Clearance Notification Processing', () => {
    const service = createService();
    const latest = service.processCustomsClearanceNotification({
      eventId: 'customs-clear-2',
      eventType: 'customs.clearance.completed',
      timestamp: new Date('2026-07-01T10:00:00.000Z'),
      source: 'ulip',
      data: { cleared: true },
      metadata: { containerId: 'CONT-CUST-2' },
    });

    const olderIgnored = service.processCustomsClearanceNotification({
      eventId: 'customs-clear-1',
      eventType: 'customs.clearance.completed',
    ...
    expect(olderIgnored.clearanceDate?.getTime()).toBe(latest.clearanceDate?.getTime());
  });

  test('Property 71: Customs Document Submission', async () => {
    let documentCalls = 0;
    const service = createService(async (url) => {
      if (url.includes('/token')) {
        return { accessToken: 'tok', tokenType: 'Bearer', expiresIn: 3600, scope: ['events:write'] };
    ...
    await service.submitCustomsDocuments('CONT-CUST-3', [
      { type: 'BOE', content: 'document-content', format: 'PDF' },
    ]);

    expect(documentCalls).toBe(1);
  });

  test('Property 72: ICEGATE Data Access', async () => {
    const service = createService();
    const icegate = await service.getICEGATEData('CONT-CUST-4');

    expect(icegate.containerId).toBe('CONT-CUST-4');
    expect(icegate.boeNumber).toBeDefined();
  });

  test('Property 73: Regulatory Hold Alert Processing', () => {
    const service = createService();
    const latest = service.processRegulatoryHoldAlert({
      eventId: 'hold-2',
      eventType: 'customs.hold.alerted',
      timestamp: new Date('2026-07-02T11:00:00.000Z'),
      source: 'ulip',
      data: { reason: 'Documentation mismatch' },
      metadata: { containerId: 'CONT-CUST-5' },
    });

    const olderIgnored = service.processRegulatoryHoldAlert({
      eventId: 'hold-1',
      eventType: 'customs.hold.alerted',
      timestamp: new Date('2026-07-02T10:00:00.000Z'),
      source: 'ulip',
      data: { reason: 'Duplicate filing' },
      metadata: { containerId: 'CONT-CUST-5' },
    });
  ...
  test('Property 74: ISO 6346 Container ID Format', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.tuple(
        ...
        async (containerId) => {
          const service = createService();
          expect(service.validateISO6346ContainerId(containerId)).toBe(true);
        }
      )
    );
  });

  test('Property 75: UN/LOCODE Location Format', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.tuple(fc.stringMatching(/^[A-Z]{2}$/), fc.stringMatching(/^[A-Z0-9]{3}$/)).map(([country, loc]) => `${country}${loc}`),
        async (locode) => {
          const service = createService();
          expect(service.validateUNLocode(locode)).toBe(true);
        }
      )
    );
  });

  test('Property 76: ISO 8601 Timestamp Format with IST', async () => {
    await fc.assert(
      fc.asyncProperty(fc.date(), async (date) => {
        const service = createService();
        const formatted = service.formatTimestampIST(date);
        expect(formatted.endsWith('+05:30')).toBe(true);
      })
    );
  });

  test('Property 77: ULIP Event Taxonomy Compliance', () => {
    const service = createService();
    expect(service.validateULIPEventTaxonomy('container.mode.changed')).toBe(true);
    expect(service.validateULIPEventTaxonomy('rail.delay.notified')).toBe(true);
    expect(service.validateULIPEventTaxonomy('bad_taxonomy')).toBe(false);
  });

  test('Property 78: ULIP JSON Schema Validation (Round-Trip)', () => {
    const service = createService();
    const ok = service.validateULIPJSONSchemaRoundTrip({
      eventId: 'roundtrip-1',
      eventType: 'ulip.event.received',
      timestamp: new Date('2026-08-01T00:00:00.000Z'),
      source: 'ulip',
      data: { message: 'ok' },
      metadata: { containerId: 'ABCD1234567' },
    });
  ...
  test('Property 79: Performance Metrics Publishing to ULIP', async () => {
    let metricCalls = 0;
    const service = createService(async (url) => {
      if (url.includes('/token')) {
        return { accessToken: 'tok', tokenType: 'Bearer', expiresIn: 3600, scope: ['events:write'] };
    ...
    await service.publishPerformanceMetrics({
      transitTimeHours: 16,
      demurrageCost: 1200,
      slotUtilization: 0.84,
  ...
  test('Property 80: ULIP Dashboard Data Access', async () => {
    const service = createService(async (url) => {
      if (url.includes('/token')) {
        return { accessToken: 'tok', tokenType: 'Bearer', expiresIn: 3600, scope: ['events:write'] };
      }
      if (url.includes('/analytics/dashboard')) {
        return { region: 'IN', nationalAverageTransitTime: 28 };
      }
      return { ok: true };
    });

    const dashboard = await service.getNationalLogisticsDashboardData('IN');
    expect(dashboard.region).toBe('IN');
  });

  test('Property 81: Report ULIP Data Inclusion', async () => {
    const service = createService(async (url) => {
      if (url.includes('/token')) {
        return { accessToken: 'tok', tokenType: 'Bearer', expiresIn: 3600, scope: ['events:write'] };
      }
      if (url.includes('/analytics/dashboard')) {
        return { benchmarkScore: 91 };
      }
      return { ok: true };
    });

    const report = await service.includeULIPDataInReport({ localKpi: 88 });
    expect(report.localKpi).toBe(88);
    expect(report.ulipBenchmarkData).toBeDefined();
  });

  test('Property 82: Anonymized Data Contribution to ULIP', async () => {
    let contributedPayload: Record<string, unknown> | undefined;
    const service = createService(async (url, body) => {
      if (url.includes('/token')) {
        return { accessToken: 'tok', tokenType: 'Bearer', expiresIn: 3600, scope: ['events:write'] };
      }
      if (url.includes('/analytics/contribute')) {
        contributedPayload = body as Record<string, unknown>;
      }
      return { ok: true };
    });

    await service.contributeAnonymizedOperationalData({
      retailerId: 'R-1',
      customerName: 'Retail Buyer',
      transitTimeHours: 18,
    });

    expect(contributedPayload?.retailerId).toBe('[REDACTED]');
    expect(contributedPayload?.customerName).toBe('[REDACTED]');
    expect(contributedPayload?.transitTimeHours).toBe(18);
  });
