...
describe('ULIPIntegrationService edge cases (Task 17.12)', () => {
  ...
  test('failed publish gets queued and can be processed later', async () => {
    let shouldFail = true;
    const service = new ULIPIntegrationService(
      { clientId: 'a', clientSecret: 'b', tokenEndpoint: 'https://ulip/token', defaultScope: ['x'] },
      {
        post: async (url: string) => {
          ...
          if (url.includes('/events') && shouldFail) {
            throw new Error('transient');
          }
          return { ok: true };
        },
      },
      { maxRetries: 0, initialBackoffMs: 1, maxBackoffMs: 1, circuitBreakerThreshold: 99, publishTimeoutMs: 1000 }
    );

    await expect(service.publishEvent({ eventId: 'queue-evt', eventType: 'x.y', timestamp: new Date(), source: 'svc', data: {}, metadata: {} }))
      .rejects.toThrow('transient');

    expect(service.getFailedRequestQueueLength()).toBe(1);

    shouldFail = false;
    const processed = await service.processFailedRequestsQueue();
    expect(processed).toBe(1);
    expect(service.getFailedRequestQueueLength()).toBe(0);
  });

  test('duplicate ULIP publish by eventId is idempotent', async () => {
    let eventCalls = 0;
    const service = new ULIPIntegrationService(
      { clientId: 'a', clientSecret: 'b', tokenEndpoint: 'https://ulip/token', defaultScope: ['x'] },
      {
    ...
    const evt = { eventId: 'idem-evt', eventType: 'ulip.event.received', timestamp: new Date(), source: 'svc', data: {}, metadata: {} };
    await service.publishEvent(evt);
    await service.publishEvent(evt);
